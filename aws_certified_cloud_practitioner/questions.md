...
- Q: True or false: only non-root volumes can be encrypted. A: False; you can encrypt both non-root and root volumes. By default, block storage is not encrypted. 
...
- Q: If you have servers in a data center and you want them to connect directly to AWS without going through the Internet, what can you use? A: Use AWS Direct Connect. If VPN is one of the choices, it is not the correct answer because VPNs will go through the Internet while DirectConnect is a private connection.
...
- Q: What is an Internet Gateway? An Internet Gateway is a part of a VPC that allows traffic from public subnets to connect to the Internet. Internet Gateways cannot be used for private subnets; you need to attach a NAT gateway or NAT instance in this case. 
...
- Q: Which service can you use to monitor, store and access log files generated by EC2 instances and on-premises servers? A: You would set up CloudWatch logs. CloudTrail logs only contain API actions.
...
- Q: How are Amazon EC2 instances billed, by the hour? Second? A: It depends. EC2 instances running Linux or Windows are billed by the second. Others are billed by the hour. Any partial hours are billed as a full hour for these other instance types.
...
- Q: How can you automate patching for RDS? A: RDS has a feature where you can schedule patching and you just need to enable it. You can't use Systems Manager for patching RDS; that only works on EC2 instances.
...
- Q: What does APN do? A: APN or AWS Partner Network helps organizations design, build and manage their AWS infrastructure. 
